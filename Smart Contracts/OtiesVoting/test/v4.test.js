...
describe("V4R:TNT20", function () {
  ...
  beforeEach(async function () {
    ...
    if(process.env.ERC20_ADDRESS !== undefined){
      rewardToken = MockERC20.attach(process.env.ERC20_ADDRESS);
    } else {
    ...
    if(process.env.ERC20_PARAMS_ADDRESS !== undefined){
      ...
      console.log("stakeTokenParams:", stakeTokenParams.address)
    }
    ...
    if(process.env.TDROP_STAKING_ADDRESS !== undefined){
      ...
      console.log("TDropStaking:", TDropStaking.address)
    }
    ...
    if(process.env.LOCKTNT20_ADDRESS !== undefined){
      ...
      console.log("lockTNT20:", lockTNT20.address)
    }
    ...
    if(process.env.V4R_ADDRESS !== undefined){
      v4r = V4R.attach(process.env.V4R_ADDRESS);
    } else {
    ...
    console.log("Updated proposal creator list");

    await lockTNT20.connect(admin).setV4rAddress(v4r.address);
    console.log("Set V4R address in LockTNT20");
    await rewardToken.connect(owner).grantMinterRole(TDropStaking.address);
    console.log("Granted Minter Role to TDropStaking");
  ...
  //
  //       await lockTNT721.connect(proposer1).lockTNT721([TOKEN_ID_1, TOKEN_ID_2]);
  //
  //       const deposits = await lockTNT721.depositsOf(proposer1.address);
  //
